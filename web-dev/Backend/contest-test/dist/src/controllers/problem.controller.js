...
export const submitDsaSolution = async (req, res) => {
    try {
        ...
        const submissionDir = path.join(process.cwd(), "temp", `${Date.now()}_${req.user.id}`);
...
`;
            ...
            try {
                const result = await runDocker(submissionDir, problem.timeLimit);
...
export const getDsaProblem = async (req, res) => {
    const promblemId = req.params.problemId;
    const problem = await prisma.dsaProblem.findUnique({
        where: {
            id: promblemId,
        },
