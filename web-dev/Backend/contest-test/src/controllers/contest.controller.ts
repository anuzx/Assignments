...
export const submitMcq = async (req: Request, res: Response) => {
  ...
  const isCorrect =
    parsedData.data.selectedOptionIndex === existQues.correctOptionIndex;
...
export const getContestLeaderboard = async (req: Request, res: Response) => {
  try {
    ...
    for (const sub of dsaSubmissions) {
      ...
      entry.dsaBest.set(
        sub.questionId,
        Math.max(currentBest, sub.pointsEarned),
      );
    }

    //  Build leaderboard array
    const leaderboard = Array.from(userMap.entries()).map(([userId, data]) => {
      const dsaPoints = Array.from(data.dsaBest.values()).reduce(
        (sum, pts) => sum + pts,
        0,
      );
